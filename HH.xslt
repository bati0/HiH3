...
>
	...
	<xsl:template match="person">
		...
		<h3><xsl:value-of select="name"/>&#xA0;<xsl:value-of select="surname"/></h3>

		Date of birth: <xsl:value-of select="birth"/>
		<xsl:if test='@alive="no"'>
			<br/>Date of death: <xsl:value-of select="death"/>
		</xsl:if>
		<br/><br/>
		<xsl:value-of select="about"/><br/>
		<xsl:apply-templates select="achievementList"/>
		<ul><xsl:for-each select="link">
			<li><xsl:call-template name="link"/></li>
		</xsl:for-each></ul>
	</xsl:template>
	...
	<xsl:template match="image">
		<img src="{@source}" alt="" class="right">
	...
	<xsl:template match="achievementList">
		<h4>Achievement</h4>
